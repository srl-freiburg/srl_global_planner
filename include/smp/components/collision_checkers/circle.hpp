...
#include <smp/components/collision_checkers/circle.h>
...
smp::collision_checker_circle<typeparams,NUM_DIMENSIONS> 
::collision_checker_circle () {
...
template< class typeparams, int NUM_DIMENSIONS > 
int smp::collision_checker_circle<typeparams,NUM_DIMENSIONS> 
:: clean_obs (int a) {
...
//  if (list_obstacles.size() == 0)
  ...
  for (typename list<region_t *>::iterator iter = list_obstacles.begin(); iter != list_obstacles.end(); iter++) {
    ...
    for (int i = 0; i < NUM_DIMENSIONS; i++) {
      ...
      if ( fabs((*state_in)[i] - region_curr->center[i]) >= (region_curr->size[1]/2+this->size_robot/2)) 
    collision = false;
    }
