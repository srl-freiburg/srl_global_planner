...
class Grid_planner{
    ...
    private:
        ...
        Tpoint old_start_; ///< @brief Start pose

        int cnt_replan; ///< @brief Counting replanning attempts with the same start pose
    ...
    public:
        ...
        */
        Grid_planner(const ros::NodeHandle& node, base_local_planner::CostmapModel* world_model, std::vector<geometry_msgs::Point> footprint_spec, costmap_2d::Costmap2DROS* costmap_ros, tf::TransformListener* listener);
        ...
        */
        ...
        /**
        * @brief Theta* search
        * @param plan, the discrete path generated by navfn
        * @return 1 if plan was found, 0 otherwise
        */
        int navfn_Search(std::vector< geometry_msgs::PoseStamped >& plan);
        /**
        * @brief A* search
        ...
        * @return true if goal was set correctly
        */
        bool setGoal(double x,double y, double theta, std::string goal_frame);
        ...
        * @return true if start was set correctly
        */
        bool setStart(double x, double y, double theta, std::string start_frame);
...
public:
        ...
        : x((int)xx), y(int(yy))
   {
    ...
    bool lineofsight(GNode *successor,GNode *parent_node)
    {
      ...
      int xa = parent_node->x;
      int ya = parent_node->y;
      int xb = successor->x;
      int yb = successor->y;
    ...
    bool lineofsightS(GNode *successor,GNode *parent_node)
    {
   ...
   // if( GetMap(x+cellwidth,y+cellheight) >=1 )
     // return true;
